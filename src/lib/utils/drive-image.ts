 ...
 */
export function processHtmlImages(htmlContent: string, driveFileMap: Record<string, string>): string {
  // Replace image src attributes
  return htmlContent.replace(/<img([^>]*)\ssrc="([^"]*)"([^>]*)>/gi, (match, beforeSrc, srcValue, afterSrc) => {
 ...
 */
export function processHtmlCssLinks(htmlContent: string, driveFileMap: Record<string, string>): string {
  // Replace CSS link href attributes (make regex case-insensitive and more flexible)
  return htmlContent.replace(/<link([^>]*)\shref="([^"]*)"([^>]*(?:type="text\/css"|rel="stylesheet")[^>]*)>/gi, (match, beforeHref, hrefValue, afterHref) => {
    // Extract filename from href (handle both relative and absolute paths)
    const filename = hrefValue.split('/').pop()?.split('?')[0] || hrefValue;
    ...
    // If we have a Google Drive file ID for this filename, replace it
    if (driveFileMap[filename]) {
      const driveUrl = `https://drive.google.com/uc?export=download&id=${driveFileMap[filename]}`;
      return `<link${beforeHref} href="${driveUrl}"${afterHref}>`;
    }
    
    // Log missing files for debugging
    console.warn(`CSS file not found in Drive: ${filename}`);
    return match;
  });
}
 ...
 */
export function extractTableOfContents(htmlContent: string): { toc: TocEntry[], htmlContent: string } {
...
function parseTocHtml(tocHtml: string): TocEntry[] {
  ...
  const liRegex = /<li class="ltx_tocentry ltx_tocentry_(\w+)"[\s\S]*?<a[^>]*href="#([^"]*)"[^>]*>[\s\S]*?<span class="ltx_tag ltx_tag_ref">([^<]*)<\/span>[\s\S]*?<span class="ltx_text[^"]*">([^<]*)<\/span>[\s\S]*?<\/a>/g;
